...
BEST_GAME=$($PSQL "SELECT MIN(number_gusses) FROM users INNER JOIN games USING(user_id) WHERE username = '$USERNAME'")
...
INSERT_GAME=$($PSQL "INSERT INTO games(number_gusses,user_id) VALUES($GUESS,$USER_ID)")